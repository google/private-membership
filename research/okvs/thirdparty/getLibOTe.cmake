...
set(DEP_NAME libOTe)
set(CLONE_DIR "${OKVS_THIRDPARTY_CLONE_DIR}/${DEP_NAME}")
set(BUILD_DIR "${CLONE_DIR}/out/build/${OKVS_CONFIG}")
...
if (NOT ${DEP_NAME}_FOUND OR LIBOTE_DEV)
    ...
    set(CONFIGURE_CMD ${CMAKE_COMMAND} -S ${CLONE_DIR} -B ${BUILD_DIR} -DCMAKE_INSTALL_PREFIX=${CMAKE_INSTALL_PREFIX}
            ...
            -DOC_THIRDPARTY_CLONE_DIR=${OKVS_THIRDPARTY_CLONE_DIR}
            )
    set(BUILD_CMD ${CMAKE_COMMAND} --build ${BUILD_DIR} --config ${CMAKE_BUILD_TYPE})
    set(INSTALL_CMD ${CMAKE_COMMAND} --install ${BUILD_DIR} --config ${CMAKE_BUILD_TYPE} --prefix ${OKVS_THIRDPARTY_DIR})


    message("============= Building ${DEP_NAME} =============")
    if (NOT EXISTS ${CLONE_DIR})
        run(NAME "Cloning ${GIT_REPOSITORY}" CMD ${DOWNLOAD_CMD} WD ${OKVS_THIRDPARTY_CLONE_DIR})
    endif ()
...
install(CODE "
    if(NOT CMAKE_INSTALL_PREFIX STREQUAL \"${OKVS_THIRDPARTY_CLONE_DIR}\")
        execute_process(
